...
func _apply_color() -> void:
	# SUPER ROBUST: finds every single MeshInstance3D in the entire drone
	...
	for mesh: MeshInstance3D in meshes:
		...
		mat.roughness = 0.2
		mat.metallic = 0.1
		mesh.material_override = mat   # This forces the color every time

	print("✅ Drone ", drone_id + 1, " colored ", drone_color)

# The rest of your functions stay exactly the same (no changes needed here)
...
# ... (the rest of _go_to_waypoint, _follow_leader_formation, _boids_behavior, _keyboard_control are unchanged from last version)
