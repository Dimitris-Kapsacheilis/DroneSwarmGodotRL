...
func _process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("toggle_camera_mode"):
		# Cycle: FOLLOW (0) -> MANUAL (1) -> STATIC (2) -> SIDE (3) -> FOLLOW (0)
...
func _static_camera_tracking(_delta: float) -> void:
	...
	size = grid_manager.grid_size.y * 1.05  # Adjust this value to zoom in or out of the area
	...
	# Position the camera well above the top boundary of the grid
	...
	# Looking straight down – use Vector3.FORWARD as temporary UP to prevent flipping
	look_at(target_position, Vector3.FORWARD)

func _side_camera_tracking(_delta: float) -> void:
	projection = Camera3D.PROJECTION_ORTHOGONAL
	# Fit the larger of height (Y) or depth (Z) so the whole grid is visible
	...
	# Place camera to the right of the grid (positive X) looking toward the center
	...
	# Look at the vertical center of the grid
