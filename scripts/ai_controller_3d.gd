...
@onready var navigator = get_parent().get_node_or_null("Navigator") if get_parent() else null
...
func _on_drone_collided(_collider: Node) -> void:
...
func get_obs() -> Dictionary:
	...
	var vel = navigator.drone.linear_velocity if navigator.drone is RigidBody3D else (navigator.drone.velocity if "velocity" in navigator.drone else Vector3.ZERO)
	...
	var all_obstacles = raw_obstacles.filter(func(n): return is_instance_valid(n) and not n.is_queued_for_deletion())
	...
	for i in range(max_tracked_obstacles):
		if i < all_obstacles.size():
			...
			if is_instance_valid(obs_node):
				...
				obstacle_obs.append_array([1.0, 1.0, 1.0, 0.0, 0.0, 0.0])
		else:
			obstacle_obs.append_array([1.0, 1.0, 1.0, 0.0, 0.0, 0.0])
...
func reset() -> void:
	...
	# Clear NFZ cache immediately so new zones are discovered fresh
	_cached_nfz_nodes.clear()
...
func _get_nfz_nodes() -> Array:
	# Purge any previously freed or queued-for-deletion nodes from cache
	_cached_nfz_nodes = _cached_nfz_nodes.filter(func(node): return is_instance_valid(node) and not node.is_queued_for_deletion())
